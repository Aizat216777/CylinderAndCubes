...
namespace MKSDK
{
    public static class ExtensionMethodsV2 
    {
        ...
        {
            return i_List.Length > 0 ? i_List[UnityEngine.Random.Range(0, i_List.Length)] : default(T);
        }
        public static T GetRandomItem<T>(this List<T> i_List) 
        {
            return i_List.Count > 0 ? i_List[UnityEngine.Random.Range(0, i_List.Count)] : default(T);
        }
